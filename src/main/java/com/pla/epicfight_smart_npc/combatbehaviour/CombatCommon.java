...
import com.pla.smart_npc.util.BowFunction;
...
public class CombatCommon {
    ...
    private static final String KEY_RANDOM_ROLL_MOVESET = "playerNpcRandomRollMoveset";
    ...
    public static boolean hasClearBowShot(MobPatch<?> mobpatch) {
        ...
        if ((mob instanceof PlayerNpcEntity) && !InventoryUtils.hasArrowAmmo(mob)) {
            return false;
        }
    ...
    public static boolean usesStepMoveset(MobPatch<?> mobpatch) {
        return !usesRandomRollMoveset(mobpatch);
    }
    ...
    private static boolean usesRandomRollMoveset(MobPatch<?> mobpatch) {
        CompoundTag persistentData = mobpatch.getOriginal().getPersistentData();
        if (!persistentData.contains(KEY_RANDOM_ROLL_MOVESET, Tag.TAG_BYTE)) {
            persistentData.putBoolean(KEY_RANDOM_ROLL_MOVESET, mobpatch.getOriginal().getRandom().nextBoolean());
        }
        return persistentData.getBoolean(KEY_RANDOM_ROLL_MOVESET);
    }
    ...
    public static boolean canEscape(MobPatch<?> mobpatch) {
        ...
        if (EscapeUtil.checkEscape(entity)) {
           return entity instanceof PlayerNpcEntity playerNpcEntity
    ...
    public static boolean isTargetingHerobrineDragon(MobPatch<?> mobpatch) {
        return ModList.get().isLoaded("annoyingvillagers") && AnnoyingVillagers.isHerobrineDragon(mobpatch.getOriginal().getTarget());
    }
    ...
    public static boolean canUsePlayerNpcLavaBucket(MobPatch<?> mobpatch) {
        Mob mob = mobpatch.getOriginal();
        LivingEntity target = mob.getTarget();
        if (target == null || !target.isAlive()) {
            return false;
    ...
    public static boolean canUsePlayerNpcWaterBucketSelfExtinguish(MobPatch<?> mobpatch) {
        return canUsePlayerNpcWaterBucketSelfExtinguish(mobpatch.getOriginal());
    }

    public static void performPlayerNpcWaterBucketSelfExtinguish(MobPatch<?> mobpatch) {
        tryPerformPlayerNpcWaterBucketSelfExtinguish(mobpatch.getOriginal());
    }

    public static void tryPerformPlayerNpcWaterBucketSelfExtinguish(Mob mob) {
        if (!canUsePlayerNpcWaterBucketSelfExtinguish(mob) || !(mob.level() instanceof ServerLevel serverLevel)) {
            return;
        }
        if (InventoryUtils.consumeItem(mob, Items.WATER_BUCKET, 1).isEmpty()) {
            return;
        }

        LivingEntityPatch<?> entityPatch = EpicFightCapabilities.getEntityPatch(mob, LivingEntityPatch.class);
        if (entityPatch != null) {
            entityPatch.playAnimationSynchronized(SmartNpcAnimations.SWING_HAND_LEFT, 0.0F);
        }

        mob.getPersistentData().putBoolean(KEY_AVNPC_WATER_BUCKET_ACTIVE, true);
        setPersistentLong(mob, KEY_AVNPC_WATER_BUCKET_COOLDOWN_UNTIL,
                mob.level().getGameTime() + AVNPC_WATER_BUCKET_MIN_COOLDOWN + new Random().nextInt(AVNPC_WATER_BUCKET_RANDOM_COOLDOWN + 1));
        equipTemporaryOffhand(mob, new ItemStack(Items.WATER_BUCKET), KEY_AVNPC_WATER_BUCKET_ORIGINAL_OFFHAND);
        ...
        new DelayedTask(AVNPC_WATER_BUCKET_ACTION_DELAY) {
            ...
            public void run() {
                if (!mob.isAlive()) {
                    mob.getPersistentData().remove(KEY_AVNPC_WATER_BUCKET_ACTIVE);
                    return;
                }

                final BlockPos placement = findSelfWaterPlacement(serverLevel, mob);
                mob.playSound(SoundEvents.BUCKET_EMPTY, 1.0F, 1.0F);
                ...
                new DelayedTask(AVNPC_WATER_BUCKET_PICKUP_DELAY) {
                    ...
                    public void run() {
                        ...
                        if (placementState.is(Blocks.WATER)) {
                            mob.playSound(SoundEvents.BUCKET_FILL, 1.0F, 1.0F);
                            serverLevel.setBlockAndUpdate(placement, Blocks.AIR.defaultBlockState());
                            recoveredSource = true;
                        } else if (ModList.get().isLoaded("annoyingvillagers") && AnnoyingVillagers.isHerobrineReplacedLiquidObsidianBlock(placementState)) {
                            AnnoyingVillagers.setBlockWithHerobrineReplacedLiquidObsidianBlock(serverLevel, placement, placementState);
                        }
    ...
    private static boolean canUsePlayerNpcWaterBucketSelfExtinguish(Mob avNpc) {
        if (!avNpc.isAlive()
                || (!avNpc.isOnFire() && !avNpc.isInLava())
    ...
    public static void performPlayerNpcLavaBucket(MobPatch<?> mobpatch) {
        if (!canUsePlayerNpcLavaBucket(mobpatch)) {
            return;
        }
    ...
    public static void placeRandomFrontWall(MobPatch<?> mobpatch) {
        ...
        if ((mob instanceof PlayerNpcEntity) && !InventoryUtils.hasPlaceableBlock(mob)) {
            return;
        }
    ...
    public static void swapToBlockToEscape(MobPatch<?> mobpatch) {
        ...
        if (entity instanceof LivingEntity livingEntity) {
            cancelCombatEvolutionGuard(mobpatch);
            if (livingEntity instanceof PlayerNpcEntity) {
